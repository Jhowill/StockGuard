...
export async function incrementUsageLimit(featureKey: PremiumFeature, delta = 1, period: 'daily' | 'monthly' = 'daily') {
  const existing = await getUsageLimit(featureKey, period);
  const key = period === 'daily' ? dateKey() : dateKey().slice(0, 7);
  const next = await saveUsageLimit({
    featureKey,
    dateKey: key,
    period,
    usedCount: (existing?.usedCount ?? 0) + delta,
    freeLimit: existing?.freeLimit ?? 0,
    rewardedLimit: existing?.rewardedLimit ?? 0,
  });
  return next;
}
